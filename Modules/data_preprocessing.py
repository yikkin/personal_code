...
def aggregate_time(dataframe , colonne_target , colonne_time):
    list_sum = []
    set_time = list(set(dataframe[colonne_time]))
...
def create_timeline(dataframe , col_time , intervalle):
    ...
    if intervalle == 'week':
        time_space = list(map(lambda x : x.strftime('%Y-%m-%d').isocalendar()[1] , df_time))
    if intervalle == 'month':
